...
### Azure Key Vaults – Infrastructure as Code (IaC)

- Any new local Azure Key Vaults **must** be created and maintained using Infrastructure as Code (IaC) techniques.
- The Infrastructure team is responsible for creating, maintaining, and deploying Key Vaults using **Bicep** files.
- Key Vaults created via IaC must **not** be reconfigured through the Azure portal dashboard or CLI, except for test purposes. The Bicep file is the single source of truth. The only exception applies to shared common key vaults.
- For security purposes, IaC Bicep files must **not** store secret values locally. Values in the Key Vaults must be imported from Azure DevOps pipeline library variable groups. Variable groups are located in Azure Devops under the Pipeline menu group.
- All values related to a local Key Vault must be stored in a **single variable group** with each environment having its own variable group.
- Variable Groups should follow the naming convention ofq-{env}-{service-name}-{service-type}-vg
