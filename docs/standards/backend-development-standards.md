...
Separate out Infrastructure and Core projects
...
In the models folder, there should generally be a folder for each database table or item represented (e.g. User)
...
### Requirement 4
Object Orientated
DTO objects should be used when transferring more complex data from a client to other services/domains.
Interfaces should be regularly used, and code should be built around using interfaces generically so that concrete classes can be replaced in the future:
Repositories and Clients always need an interface
Shared attributes and functionality between models should always have an interface that is then implemented across all models that can use it
