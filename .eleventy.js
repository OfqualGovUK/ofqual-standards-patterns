...
// NEW: rehype plugins to customize MDX output
...
function injectGitSha(eleventyConfig, gitHubRepositoryUrl) {
  ...
  eleventyConfig.addGlobalData("gitHashPath", gitHubRepositoryUrl + "/blob/" + latestGitCommitHash + "/docs");
}
...
export default async function (eleventyConfig) {
    ...
    footer: {
      ...
      meta: {
        items: [
          ...
          { href: gitHubRepositoryUrl, text: "GitHub repository" },
        ],
      },
    },
  ...
  // GOV.UK class map applied to MDX via rehype-add-classes
  const govukClassMap = {
    // Typography
    p: "govuk-body",
    "h1,h2,h3,h4,h5,h6": "govuk-heading",
    h1: "govuk-heading-xl",
    h2: "govuk-heading-l",
    h3: "govuk-heading-m",
    h4: "govuk-heading-s",
  ...
  // MDX with full front‑matter compatibility + rehype transforms
  eleventyConfig.addExtension("mdx", {
    compile: async (str, inputPath) => {
      ...
      const { default: MDXContent } = await evaluate(str, {
        ...
        // Rehype plugin to add GOV.UK styles to MDX processed files
        rehypePlugins: [
          ...
          [
            ...
            {
              behavior: "append",
              properties: { class: "govuk-heading__permalink", ariaHidden: true },
            },
          ],
          [rehypeAddClasses, govukClassMap],
        ],
      });
  ...
  await Promise.all(
    fs
      .readdirSync(path.join(process.cwd(), "lib", "filters"))
